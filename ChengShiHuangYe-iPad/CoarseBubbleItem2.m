...
@implementation CoarseBubbleItem2
...
-(instancetype) initWithRelyItem:(UIView*)relyItem
{
    if (self=[super init])
    {
        ...
        CGFloat positionY=relyItem.frame.origin.y+self.space+100;
    
        _bubble=[[UIButton alloc] initWithFrame:CGRectMake(positionX, positionY, 181, 100)];
        [_bubble setImage:[UIImage imageNamed:@"coarseBubble2"] forState:UIControlStateNormal];
